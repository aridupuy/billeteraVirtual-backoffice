...
//            else
//                var_dump ($frame->get_node());
        }
        if(get_class($frame)=="Dompdf\FrameDecorator\Page"){
            $frame= $original_frame;
            var_dump(get_class($frame));
            foreach ($frame=$frame->get_children() as $frame){
                if(!is_array($frame) and $frame->is_table())
                    return $frame;
            }
        }
        if(get_class($frame)=="Dompdf\FrameDecorator\Block"){
            $frame= $original_frame;
            foreach ($frame=$frame->get_children() as $frame){
                var_dump(get_class($frame));
                if(!is_array($frame) and $frame->is_table())
