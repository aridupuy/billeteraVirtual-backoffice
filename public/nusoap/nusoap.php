...
class nusoap_base
{
    ...
    public function serializeEnvelope($body, $headers = false, $namespaces = array(), $style = 'rpc', $use = 'encoded', $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/')
    {
        ...
        if ($encodingStyle) {
            if($this->soapPrefix == "soapenv")
                $ns_string = " SOAPENV:encodingStyle=\"$encodingStyle\"$ns_string";
            else $ns_string = " SOAP-ENV:encodingStyle=\"$encodingStyle\"$ns_string";
        }
        ...
        // serialize envelope
        if($this->no_xsi){
            $body = str_replace(' xsi:type="xsd:string"', "", $body);
        }
        if($this->soapPrefix == "soapenv")
        return
...
class nusoap_server extends nusoap_base
{
     ...
     * @access   public
     */
    public function register($name, $in = array(), $out = array(), $namespace = false, $soapaction = false, $style = false, $use = false, $documentation = '', $encodingStyle = '', $otherinfo = '')
    {
        global $HTTP_SERVER_VARS;
...
//            var_dump(utf8_encode($xml));
            if (!xml_parse($this->parser, utf8_encode($xml), false)) {
