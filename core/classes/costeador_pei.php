...
/**
 * Description of costeador_pei
 *
 * @author ariel
 */
class costeador_pei extends Costeador{
    const id_mp=222;
    ...
    public function actualizar_transaccion_pei(Pei_transaccion $transaccion) {
        $transaccion->set_costeado("true");
        return $transaccion->set();
    }
    public function costear(Pei_transaccion $sabana) {
        Model::StartTrans();
        $entidad = $this->obtener_entidad($sabana);
        $transaccion = Transaccion_pei::factory($entidad);
/*        var_dump($sabana);
        return;
*/
        //switch (get_class($entidad)){
            //case Barcode::class:
                $fecha=$sabana->get_fecha_gen();
          //      break;
          //  case Debito_tco::class:
        ...
        if(!$fecha_aux){
             $fecha_aux= DateTime::createFromFormat("Y-m-d H:i:s.u",$fecha);
         }

        $fecha=$fecha_aux;
        if(!$transaccion->crear($entidad->get_id_marchand(), Mp::PEI, $sabana->get_monto(),$fecha , $entidad->get_id()))
        {
            Model::FailTrans();
            developer_log("Error al crear la transaccion.");
        }
        if($this->actualizar_entidad($entidad)){
            if(!$this->actualizar_transaccion_pei($sabana)){
                Model::FailTrans();
...
//    }
	   ...
	   Model::CompleteTrans();
        ...
        if ($recordset and $recordset->RowCount()>0) {
            ...
            foreach ($recordset as $row) {
                $sabana=new Pei_transaccion($row);
    ...
    protected function obtener_recordset() {
        return Pei_transaccion::select_pagos_a_costear();
    }
    protected function obtener_entidad(Pei_transaccion $pago){
        $barcode = new Barcode();
    ...
    protected function obtener_semaforo() {
        $recordset = Mp::semaforo_libre_para_costear(Mp::PEI);
