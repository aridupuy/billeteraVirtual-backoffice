...
require_once "Image_Barcode.php";
...
class Image_Barcode_code128 extends Image_Barcode
{
     ...
     *
     */
    //function &draw($text, $imgtype = 'png', $bbh=60)
    function draw($text, $nada='', $imgtype = 'png', $bbh=60)
    {
        developer_log("aca121");
        $this->_barcodeheight=$bbh;
//        $this->_barcodeheight=80;
...
//        var_dump($barcodewidth, $barcodelongheight+ imagefontheight($this->_font)+1);
...
//        exit();
        ...
        // CD especial.
        $longcodeis=preg_replace("/(\d\d\d)(\d\d\d\d)(\d\d\d\d)(\d\d\d\d)(\d\d\d\d\d\d)(\d\d\d\d\d\d\d)(\d)/",
                             "\\1 \\2 \\3 \\4 \\5 \\6 \\7",
                             $text);
        ...
        imagestring(
            $img,
            $this->_font,
            $barcodewidth / 2 - strlen($longcodeis) / 2 * (imagefontwidth($this->_font)),
