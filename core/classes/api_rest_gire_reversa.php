 ...
 * @author ariel
 */
class Api_rest_gire_reversa extends Api_rest_gire {
    ...
    public function run() {
        ...
        if (($barcode = $this->obtener_barcode($codigo_barras)) != false) {
            developer_log($barcode->get_id());
            $moves = $this->obtener_moves($barcode->get_id(), $variables["fecha_hora_operacion"], $importe);
            $id_moves = $moves[0];
            ...
            if($this->ya_reversado($barcode->get_id(), $importe)){
                Model::FailTrans();
                throw new ExceptionApiGire("Error pago ya reversado", 3);
            }
            
            if(!$trans->reversar($move)){
                Model::FailTrans();
                throw new ExceptionApiGire("Error al reversar el pago", 10);   
            }
...
//            $sabana->set_xml_extra("Sabana ingresada por CashIn");
//            if (!$sabana->set()) {
//                Model::FailTrans();
//                throw new ExceptionApiGire("Error al guardar el pago", 10);
//            }
            if (!Model::HasFailedTrans() and Model::CompleteTrans())
                return $this->responder($variables);
        }
        Model::FailTrans();
        throw new ExceptionApiGire("Error interno del sistema", 10);
    }

    private function ya_reversado($id_barcode, $importe){
        $recorset = Moves::buscar_reversos($id_barcode, $importe);
        return $recorset->rowCount()>0;
    }
    
    private function obtener_moves($id_barcode, $fecha_pago, $importe){
        $recordset = Moves::obtener_pago_de_barcode($id_barcode, $fecha_pago, $importe);
        if($recordset->rowCount() < 1){
            throw new ExceptionApiGire("Error de validación del código de cliente: Barcode invalido", 8);
        }
        $moves = $recordset->fetchRow();
        return $moves;
    }
    ...
    private function responder($variables) {
        /*
...
//        $sabana->get($id_sabana);
        ...
        $respuesta["barra"] = $variables["barra"];
        $respuesta["fecha_hora_operacion"] = $variables["fecha_hora_operacion"];
