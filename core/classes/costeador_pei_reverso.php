...
/**
 * Description of costeador_pei_reverso
 *
 * @author ariel
 */
class costeador_pei_reverso extends Costeador{
    //put your code here
    ...
    public function actualizar_transaccion_decidir(Pei_transaccion $transaccion) {
        $transaccion->set_costeado("true");
        $transaccion->set_reversado("true");
        return $transaccion->set();
    }
    public function reversar(Pei_transaccion $sabana) {
        Model::StartTrans();
        ...
        if(!$fecha_aux){
            $fecha_aux= DateTime::createFromFormat("Y-m-d H:i:s.ue",$fecha);
        }
        $fecha=$fecha_aux;
        $moves = $this->obtener_moves_a_reversar($entidad->get_id_marchand(), Mp::PEI, $sabana->get_monto(), $entidad->get_id(),$id_entidad);
        if(!$moves){
            Model::FailTrans();
...
//    }
    ...
    protected function obtener_recordset() {
        return Pei_transaccion::select_pagos_a_reversar();
    }
